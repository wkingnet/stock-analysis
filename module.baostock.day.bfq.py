...
def stock_get_lastdate(stockcode):
    ...
    """
    file = ucfg.baostock['csv_day_bfq'] + os.sep + stockcode
...
# 判断目录和文件是否存在，存在则直接删除
if os.path.exists(ucfg.baostock['csv_day_bfq']):
    choose = input("文件已存在，输入 y 删除现有文件并重新生成完整数据，其他输入则附加最新日期数据: ")
    if choose == 'y':
        for root, dirs, files in os.walk(ucfg.baostock['csv_day_bfq'], topdown=False):
            for name in files:
                os.remove(os.path.join(root,name))
            for name in dirs:
                os.rmdir(os.path.join(root,name))
        try:
            os.mkdir(ucfg.baostock['csv_day_bfq'])
...
for i in stocklist:
    ...
    csv_file = ucfg.baostock['csv_day_bfq'] + os.sep + i + '.csv'
