...
- 数据存储方式优化。目前使用csv文件，性能略差。但这个也不是特别需要。【2021年2月10日19:03:38完成，使用pickle格式保存】
- 
...
盘中或盘后，运行xuangu.py，会自动调用celue.py执行策略选股并显示结果。

#### 盘后选股

盘后离线数据选股。当日16点以后，自己手动控制通达信更新完当日数据，运行xuangu.py进行选股。xuangu.py里不指定日期的话，是按照当天数据选股的。如果指定日期，是按照日期区间的数据选股。

#### 盘中选股

按照实时数据选股。程序会获取当日实时最新行情，整合到具体股票日线数据中（不保存到文件，只在内存操作），并判断是否需要前复权（前复权的特性是如果今天除权，那么以前的数据全需要变）。
