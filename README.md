...
数据更新且celue.py写好后，就可以选股了。

#### 盘后选股

当日16点以后，自己通达信更新完，运行xuangu.py进行选股。xuangu.py里不指定日期的话，是按照当天数据选股的。

#### 盘中选股

仍然运行xuangu.py进行选股。程序会获取当日实时最新行情，整合到具体股票日线数据中（不保存到文件，只在内存操作），判断是否需要前复权（前复权的特性是如果今天除权，那么以前的数据全需要变）。是按照实时数据选股。
