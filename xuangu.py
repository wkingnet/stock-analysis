...
def run_celue1(stocklist, df_dict):
    tq = tqdm(stocklist[:])
    for stockcode in tq:
        tq.set_description(stockcode)
        if 'df_today' in dir():  # 更新当前最新行情，否则用昨天的数据
            df_dict[stockcode] = func_TDX.update_stockquote(stockcode, df_dict[stockcode], df_today)
        df_dict[stockcode]['date'] = pd.to_datetime(df_dict[stockcode]['date'], format='%Y-%m-%d')  # 转为时间格式
        df_dict[stockcode].set_index('date', drop=False, inplace=True)  # 时间为索引。方便与另外复权的DF表对齐合并
        celue1 = CeLue.策略1(df_dict[stockcode], start_date=start_date, end_date=end_date, mode='fast')
        if not celue1:
            stocklist.remove(stockcode)
            del df_dict[stockcode]
    return stocklist


def run_celue2(stocklist):
    tq = tqdm(stocklist[:])
    for stockcode in tq:
        tq.set_description(stockcode)
        if '09:00:00' < time.strftime("%H:%M:%S", time.localtime()) < '16:00:00':
            df_today_code = df_today.loc[df_today['code'] == stockcode]
            df_dict[stockcode] = func_TDX.update_stockquote(stockcode, df_dict[stockcode], df_today_code)
            ...
            if now_date in df_gbbq.loc[df_gbbq['code'] == stockcode]['权息日'].to_list():
                df_dict[stockcode] = func_TDX.make_fq(stockcode, df_dict[stockcode], df_gbbq, cw_dict)
        celue2 = CeLue.策略2(df_dict[stockcode], HS300_信号, start_date=start_date, end_date=end_date).iat[-1]
        if celue2:
...
if __name__ == '__main__':
    ...
    stocklist = run_celue1(stocklist, df_dict)
    # else:
    ...
    #             pool_result.append(p.apply_async(run_celue1, args=(stocklist[i * div:(i + 1) * div], df_dict)))
    #         else:
    #             print(i, i * div, (i + 1) * div + mod)
    #             pool_result.append(p.apply_async(run_celue1, args=(stocklist[i * div:(i + 1) * div + mod], df_dict)))
    #
    ...
    starttime_tick = time.time()
    stocklist = run_celue2(stocklist)
